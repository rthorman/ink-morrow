...
- Export is EPUB (dependency-free zip writer in backend/src/epub.js)- Narration (implemented per SCRIBETRIBE-STREAMING-TTS-IMPLEMENTATION.md): MP3 pass-through streaming (axios responseType:'stream' -> express pipe, upstream aborted via res.on('close')); X-Generation-Id captured; in-memory audio cache keyed sha256(text)+model+voice (replays never re-bill); authoritative cost via GET /generation reconciled in /api/ai/generation-cost (server-cached per id); client bills once per generation id (Set guard); pages >16k chars → 413 (segmentation deliberately deferred); CSP includes media-src 'self' blob:
