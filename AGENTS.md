...
- Schema 16 living campaign state is optional and revisioned. Owner records
  and current continuity projections share one bounded read model without
  copying or mutating extracted deltas. Recaps prioritize Main, Supporting,
  then Background and cap records per tier. Explicit Play analysis produces
  source-quoted proposals only; each item requires an owner Add action. Paid
  requests are idempotent, restart-reconciled, stale-checked, and accounted.
...
- Schema 18 solo tools are reusable Story-owned dice, likelihood oracle,
  weighted table, deck, user-field, and progress-clock definitions. Runs use
  `node:crypto` application code only and create immutable branch-aware
  `play_tool_records`; the Scribe may receive bounded recorded summaries but
  never rolls or changes a result. Tool definitions/state always archive;
  frozen results require working history and remap with scenes, sessions,
  branches, and tools. Full behavior: `docs/play.md`.
...
- Per-page AI accounting: model, prompt/completion tokens, cost_usd on story_pages; stories expose total_cost_usd; the enabled cost ticker remains in the private application shell across every surface; frontend settings (localStorage im-settings): model picker (GET /api/models proxy marks the server default and exposes `reasoning_efforts`, `reasoning_default`, and `reasoning_mandatory`; the UI resolves the explicit selection OR server default, renders only that model's declared subset of none|minimal|low|medium|high|xhigh|max, and sends it as `reasoning_effort` on generate/regenerate/preview), words per page (50–2000, scales max_tokens), story font + text size, cost ticker (default on), narration model/voice pickers with ≈per-page cost labels. The retired writing-background toggle and `scriptoriumBg` setting are ignored and must not return.
