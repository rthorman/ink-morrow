...
describe('5.0 fresh data boundary', () => {
  ...
  test('new stores have the 5.0 family and independent default storage paths', () => {
    const db = createDb(':memory:'); handles.push(db); expect(schemaIdentity(db)).toMatchObject({ family: 'ink-morrow-5', version: 23 }); expect(db.prepare('PRAGMA application_id').get().application_id).toBe(0x494D3530);
  ...
  test('valid 5.0 state committed only in WAL is recognised without touching source and recovers normally', () => {
    ...
    const before = snapshot(file); expect(inspectExistingDatabase(file)).toMatchObject({ kind: 'recognized', version: 23 }); expect(snapshot(file)).toEqual(before);
