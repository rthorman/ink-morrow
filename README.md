...
- **Publication core** — freeze reviewed display prose, hierarchy, metadata, front/back matter, scene breaks, and selected placed art into one immutable allowlisted document, then render semantically checked DOCX, ODT, RTF, standalone HTML, Markdown, plain text, or documented JSON without exposing prompts, continuity, recovery, costs, credentials, or working history
...
| GET | `/api/publications/:snapshotId/formats/:format` | Render the snapshot as `docx`, `odt`, `rtf`, `html`, `md`, `txt`, or `json` |
