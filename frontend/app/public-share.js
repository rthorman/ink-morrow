...
function matter(section, className) {
  ...
  for (const item of section) {
    ...
    for (const paragraph of String(item.text || '').split(/\n{2,}/)) {
      if (paragraph.trim()) wrapper.appendChild(node('p', '', paragraph.trim()));
    }
  }
  return wrapper;
}
...
function render(publication) {
  ...
  const front = matter(documentValue.front_matter, 'share-matter share-matter--front');
    ...
    for (const chapter of volume.chapters) {
      ...
      for (const page of chapter.pages) {
        ...
        for (const block of page.blocks) {
          if (block.type === 'paragraph') pageSection.appendChild(node('p', '', block.text));
          else if (block.type === 'scene_break') pageSection.appendChild(node('hr', 'share-scene-break'));
            ...
            pageSection.appendChild(figure);
          }
        }
  ...
  const back = matter(documentValue.back_matter, 'share-matter share-matter--back');
