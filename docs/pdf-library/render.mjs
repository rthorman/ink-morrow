...
try {
  for (const book of books) {
    const markdown = await fs.readFile(path.join(here,book.source),'utf8');
    ...
    const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Ink Morrow 4.0 - ${book.title}</title><style>${css}</style></head><body><section class="cover"><img src="${book.cover}" alt=""><div class="cover-copy"><div class="cover-kicker">Ink Morrow 4.0 Documentation Library</div><h1>${book.title}</h1><div class="cover-subtitle">${book.subtitle}</div><div class="cover-meta">${book.audience} &nbsp; / &nbsp; September 2026</div></div></section><main>${bodyWithToc}</main></body></html>`;
    ...
    await page.pdf({path:output,format:'A4',printBackground:true,preferCSSPageSize:true,tagged:true,outline:true,displayHeaderFooter:true,headerTemplate:'<div style="font:7px Georgia;color:#766a72;width:100%;padding:0 15mm;text-align:right">INK MORROW 4.0</div>',footerTemplate:'<div style="font:7px Georgia;color:#766a72;width:100%;padding:0 15mm;display:flex;justify-content:space-between"><span>WHERE STORIES GROW CLAWS</span><span class="pageNumber"></span></div>',margin:{top:'14mm',right:'15mm',bottom:'13mm',left:'15mm'}});
    await page.close();
